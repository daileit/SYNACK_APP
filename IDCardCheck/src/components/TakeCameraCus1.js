...
import { nstyles, nwidth, nheight, paddingBotX, Height, Width } from '../styles/styles'
...
var check = 1;
export default class TakeCameraCus1 extends Component {
  constructor(props) {
    super(props);
    //--Options default
    this.option = {
  ...
  render() {
    ...
    let heoghtIMGConten = widthIMGConten * 571 / 1080;
    return (
      <View style={stTakeCamera.container}>
        ...
        >
          ...
          <View style={[nstyles.position, { bottom: 0 }]}>
            ...
            <Image style={{ width: widthIMGConten, marginHorizontal: -25, height: heoghtIMGConten }}
              source={Images.icFocusCard} resizeMode='contain' />
            <View style={{ backgroundColor: colors.black_30, height: '40%', justifyContent: 'flex-end', alignItems: 'center' }}>
              <Text style={{
                color: colors.white, textAlign: 'center',
                fontStyle: 'italic', marginBottom: 50
              }}>
                {RootLang.lang.Placethe_frame}
            </Text>
  ...
  cropImg = async (item, downSize = 1) => {
    console.log('cropImg1:', item);
    let uri = item.uri;
    let width = item.width;
    let height = item.height;
    let widthCrop = width - width * 0.06;
    let heightCrop = widthCrop * 54 / 86;

    let tempX = width * 0.03;
    let tempY = height - height * 0.4 - heightCrop;
    if(Platform.OS=='android'){
      widthCrop = width - width * 0.32;
      heightCrop = widthCrop * 54 / 86;

      tempX = width * 0.16;
      tempY = height - height * 0.4 - heightCrop;
    }
    ...
    console.log('cropImg2:', { uri: uriReturn, width: widthCrop, height: heightCrop });
